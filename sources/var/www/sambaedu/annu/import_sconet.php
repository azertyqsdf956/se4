 ...
 * Page d'import des comptes depuis les fichiers CSV/XML de Siecle/STS
 ...
 * @note
 * @su www-admin php-fpm-cli /usr/share/se3/script/import_comptes.php
...
if (have_right($config, "Annu_is_admin")) {
    ...
    if (! isset($_POST['is_posted'])) {
        ...
        if ($config['imprt_cmpts_en_cours'] == "y") {
            ...
            $test_tab = search_ad($config, "*", "user", $config['dn']['trash']);
            if (count($test_tab)) {
...
decompte(cpt);
        ...
        if (strlen(preg_replace("/_/", "", $prefix)) == 0) {
            $prefix = "";
        }
        if (strlen($prefix) > 0) {
            $prefix = $prefix . "_";
        ...
        fclose($fich);
        // chmod("/varz/remote_adm/import_comptes.sh",750);
        ...
        echo "</b>";
        if ($prefix != "") {
            echo " avec le pr&#233;fixe <b>$prefix</b>";
        }
        echo ".</p>\n";

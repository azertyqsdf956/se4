...
// Verifie les droits
if ((have_right($config, "computers_is_admin")) or (have_right($config, "parc_can_view")) or (have_right($config, "parc_can_manage")) or (have_right($config, "inventaire_can_read"))) {
	...
	if ($acces_restreint)  {  if ((!have_delegation($config,$parc,"manage")) and (!have_delegation($config, $parc, "view"))) { exit; } }
	...
	case "stop":
		if (($parc)  and ($parc<>"SELECTIONNER")) {
			if ($acces_restreint)  {  if ((!have_delegation($config,$parc,"manage")) and (!have_delegation($config,$parc,"view"))) { continue; } }
		...
		$list_parcs = list_parc($config);
			...
			if ($acces_restreint=="1") {
				...
				foreach ($list_delegate as $info_parc_delegate) {
					echo "<option value=\"".$info_parc_delegate['cn']."\"";
					if ($parc==$info_parc_delegate['cn']) { echo " selected"; }

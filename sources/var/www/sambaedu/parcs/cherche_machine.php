...
include ("siecle.inc.php"); // Pour les recherches de doublons
...
if (have_right($config, "computers_is_admin")) {
    ...
    if ($_POST['sansparc'] == "oui") {
        ...
        // echo "<table>\n";
        $list_computer = search_ad($config, "*", "machines");
        ...
        if (count($list_computer) > 0) {
            ...
            for ($loopa = 0; $loopa < count($list_computer); $loopa ++) {
                if ($list_computer[$loopa]['cn'] != $netbios_name) {
                    ...
                    $exist_parc = list_parc($config, $list_computer[$loopa]["cn"]);
	...
	function UncheckAll(){
        ...
        if ($mpenc != "") {
            $computer = search_machine($config, $mpenc, true);
            if ((count($computer) == 1)) {
                $ipHost = avoir_ip($mpenc);
                echo "<a href=show_histo.php?selectionne=2&mpenc=$mpenc>$mpenc</a> (" . $computer['iphostnumber'] . ") se trouve dans les parcs&nbsp;: ";
                for ($loopa = 0; $loopa < 1; $loopa ++) {
                    // echo $computer[$loopa]["cn"];
                    echo "<br /><br />\n";
                    $list_parcs = list_parc($config, $computer['cn']);
                    if (count($list_parcs) > 0) {
                        for ($loop = 0; $loop < count($list_parcs); $loop ++) {
                            $parc = $list_parcs[$loop]["cn"];
                            echo "<A HREF=\"show_parc.php?parc=$parc\">" . $list_parcs[$loop]["cn"] . " type : " . $liste_parc[$loop]['type'] . "</A>";
                            echo "<br />\n";
                        }
                    }
                    if (count($list_parcs) == 0) {
        ...
        if ($mpenc != "") {
            $list_computer = search_machine($config, $mpenc);
            if (count($list_computer) > 0) {

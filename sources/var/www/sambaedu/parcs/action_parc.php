...
$prefix = "action_parc";
// $lang="en";
require_once ("messages/".$config['lang']."/" . $prefix . "_messages.php");
// ##########
// aide
...
// Verifie les droits
if ((have_right($config, "computers_is_admin", $login) == "Y") or (have_right($config, "parc_can_view", $login) == "Y") or (have_right($config, "parc_can_manage", $login) == "Y") or (have_right($config, "inventaire_can_read", $login) == "Y")) {

    echo "<h1>" . gettext("$action_parc[pageTitre]") . "</h1>";

    // *****************cas des parcs delegues***********************************/
    if ((have_right($config, "computers_is_admin", $login) == "N") and ((have_right($config, "parc_can_view", $login) == "Y") or (have_right($config, "parc_can_manage", $login) == "Y"))) {
        echo "<h3>" . gettext($action_parc['msgDelegationAccept']) . "</h3>";
        $acces_restreint = 1;

        $list_delegate = list_delegations($config);
    ...
    switch ($action) {
        ...
        case "detail":
            ...
            if (count($list_parcs) > 0) {
                ...
                if ($delegate == "yes") {
                    ...
                    foreach ($list_delegate as $info_parc_delegate) {
                        ...
                        if ($parc == $info_parc_delegate['cn']) {
                            echo " selected";
                        }
                        echo ">".$info_parc_delegate['cn']."</option>\n";
                    }
                } else {
            ...
            if (($parc != "") && ($parc != "SELECTIONNER")) {
                ...
                $niveau_delegation = type_delegation($config, $parc);
                ...
                if (count($mp) > 0) {
                    ...
                    for ($loop = 0; $loop < count($mp); $loop ++) {
                        ...
                        $mp_curr = search_machines($config, $mp_en_cours);
                        ...
                        if (isset($mp_curr["ipHostNumber"])) {
                            $iphost = $mp_curr["ipHostNumber"];

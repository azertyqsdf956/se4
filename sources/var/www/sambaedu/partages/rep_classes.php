...
if ((is_admin($config, "se3_is_admin", $login) == "Y") or ( is_admin($config, "annu_is_admin", $login) == "Y")) {
    ...
    if (isset($_POST['create_folders_classes']) && isset($_POST['new_folders_classes'])) {
        $new_folders_classes = (isset($_POST['new_folders_classes'])?$_POST['new_folders_classes']:"");
    ...
    if (isset($_POST['delete_folders_classes'])) {
    ...
    if (isset($_POST['refresh_folders_classes'])) {
    ...
    if (isset($_POST['refresh_classes']) || isset($_POST['clean_classes'])) {
        ...
        if (count($refresh_RessourcesClasses) > 0) {
            for ($loop = 0; $loop < count($refresh_RessourcesClasses); $loop++) {
                ...
                elseif (isset($_POST['clean_classes'])) {
                    echo "<b>Nettoyage de la classe : $Niveau</b><br />\n";
                    $commande = "/usr/bin/sudo /usr/share/sambaedu/scripts/cleanClasses.pl $Niveau ";
                    my_echo_debug($commande);
                    system($commande);
                }
            }
...
//    $ipHostNumber = $serveur[0]["ipHostNumber"];
    ...
    if (count($list_new_classes) > 15)
        $size = 15;
    else
        $size = count($list_new_classes);
    if (count($list_new_classes) > 0) {
        ...
        if (isset($create_folders_classes) && count($new_folders_classes) == 0) {
    ...
    if (count($RessourcesClasses) > 15)
        $size = 15;
    else
        $size = count($RessourcesClasses);
...
//        	echo "<input type=\"hidden\" name=\"delete_folders_classes\" value=\"true\">\n";
        ...
        echo "</form>\n";
        // V&#233;rification selection d'au moins une classe
        if ((isset($refresh_classes) || isset($delete_folders_classes) || isset($clean_classes)) && count($old_RessourcesClasses) == 0) {
            echo "<div class='error_msg'>" . gettext("Vous devez s&#233lectionner au moins une classe !") . "</div>\n";
        }
    }
    /* 	echo "</td><td align=\"center\">\n";
...
//        $CHECKED = "checked";
//    }
    $CHECKED= ($acl_group_profs_classes == "1"?"checked":"");

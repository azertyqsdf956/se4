...
if (d) {d.style.display='block';list}
}
//--></script><div id=\"menu\">
  ";
    if ($parc) {
      ...
      <ul class=ssmenu onmouseout=\"javascript:montre('');\">\n";
        ...
        if (count($list_parcs) > 0) {
            for ($loop = 0; $loop < count($list_parcs); $loop ++) {
                if ($acces_restreint) {
                    if ((! have_delegation($config, $list_parcs[$loop]["cn"], "manage")) and (! have_delegation($config, $list_parcs[$loop]["cn"], "view"))) {
                        continue;
                    }
                }
                if ($list_parcs[$loop]["cn"] != $parc) {
                    echo "<li class=ssmenu onmouseout=\"javascript:montre('');\"><a href=\"action_parc.php?parc=" . $list_parcs[$loop]["cn"] . "\"><img src=\"../elements/images/typebullet.png\" width=\"30\" height=\"11\" border=\"0\">" . $list_parcs[$loop]["cn"] . "</a></li>\n";
                }
            }
        }
        echo "</ul>
 ...
 * @return
 */
function avoir_systemid($nom_machine)
{ // retourne l'ID de $nom_machine ou 0 a partir de la table hardware
...
bitmapcachepersistenable:i:1";
        ...
        if (file_exists($fichier_rdp))
            unlink($fichier_rdp);

        $get = fopen($fichier_rdp, "w+");
        fputs($get, $ligne);
        fclose($get);
        $ts = "<a href=\"tsvnc.php?machine=$ip&action=ts&file=$fichier_rdp\"" . "onmouseout=\"UnTip();\" onmouseover=\"Tip('" . $action_parc['msgTsWarning'] . "',WIDTH,250,SHADOW,true,DURATION,5000);\"" . "><img src=\"../elements/images/monitorTS.gif\" alt=\"\" title=\"terminal server\" width=\"30\" height=\"30\" border=\"0\" /></a>\n";
        return $ts;
    }
}
...
DSMPlugin=";
        ...
        if (file_exists($fichier_vnc))
            unlink($fichier_vnc);

        $get = fopen($fichier_vnc, "w+");
        fputs($get, $ligne);
        fclose($get);
        $vnc = "<a href=\"tsvnc.php?machine=$ip&action=vnc&file=$fichier_vnc\"" . "onmouseout=\"UnTip();\" onmouseover=\"Tip('" . $action_parc['msgVncWarning'] . "',WIDTH,250,SHADOW,true,DURATION,5000);\"" . "><img src=\"../elements/images/monitorVNC.gif\" alt=\"\" title=\"terminal server\" width=\"30\" height=\"30\" border=\"0\" /></a>\n";
...
function heure_deroulante($parcf, $jourf, $type_actionf)
{
    ...
    for ($if = 0; $if < 24; $if ++) {
        ...
        for ($jf = 0; $jf < 4; $jf ++) {
            $mf = $jf * 15;
            $selectedf = "";
            if (($if == $timef[0]) and ($mf == $timef[1])) {
                $selectedf = "SELECTED";
            } // elseif ($heure_act==$i-1) { $selected="SELECTED"; }
            if ($mf < 10) {
                echo "<option $selectedf >$if:0$mf</option>\n";
            } else {
                echo "<option $selectedf>$if:$mf</option>\n";
            }
            $selectedf = "";
        }
    }
...
function detail_parc($parc)
{
    ...
    $mp_all = list_members_parc($config, $parc);
    ...
    if (count($mp) > 0) {
        ...
        for ($loop = 0; $loop < count($mp); $loop ++) {
            ...
            $mp_curr = search_machine($config, $mp_en_cours);
            ...
            if ($suisje_printer != "1") {
                ...
                if (isset($mp_curr[0]["ipHostNumber"])) {
                    ...
                    if ($ping) {
                        ...
                        if ((! $ts) and (! $vnc)) {
                            echo "<img type=\"image\" border=\"0\" src=\"../elements/images/disabled.png\">\n";
                        }
                    } else {
                        echo "<img type=\"image\" border=\"0\" src=\"../elements/images/disabled.png\">\n";
                    }
...
function detail_parc_printer($parc)
{
    ...
    $mp_all = list_members_parc($config, $arc);
...
function update_wpkg()
{
    ...
    if (file_exists($wpkgUpdateHostsProfiles))
        exec("$wpkgUpdateHostsProfiles '$computersRdn' '$parcsRdn' '$ldap_base_dn'");
    ...
    if (file_exists($wpkgUpdateDroit))
        exec("$wpkgUpdateDroit");
}
    ...
    if (! ($data = apc_fetch('smb_login'))) {
        ...
        foreach ($resultat as $ligne) {
            ...
            if ((count($table) == 8) && ($table[1] != root) && ($table[1] != nobody) && (preg_match("/(.+)\\$/", $table[1]) === 0)) {
                $mp_curr = search_machine($config, $table[3]);
